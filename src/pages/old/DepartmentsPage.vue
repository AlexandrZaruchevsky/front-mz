<template>
  <div class="relative flex h-full justify-center items-start">
    <card-z-v2 class="w-full h-full" :cardData="cardData" header="Departments" navigation searchable editable footer>
      <template #body>
        <div class="p-2 flex flex-col gap-0.5">
          <div v-for="dep in deps" :key="dep.id"
            class="flex px-2 rounded hover:bg-slate-200 hover:cursor-pointer border-b-2 border-dotted"
            @click="$router.push(`/admin/departments/${dep.id}`)">
...
const { cardData, deps } = storeToRefs(useDepartmentStore());
