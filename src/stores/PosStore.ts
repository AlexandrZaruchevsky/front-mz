...
export const usePosStore = defineStore('posStore', () => {
  ...
  const page = ref<PositionPage>(new PositionPage());
  const entityService = new PosService("positions");
  ...
  const entities = computed<Array<Position>>(() => page.value.content);
  const entity = ref<Position>(new Position());
  ...
  const addEntity = async () => {
    router.push({ path: "/admin/positions/add" })
  }
